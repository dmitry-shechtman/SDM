...
static void gui_mbox_draw_buttons() {
    ...
    {
     if(gPlayRecMode>0)
        {cl = MAKE_COLOR((mbox_button_active==i)?COLOR_RED:COLOR_BLACK, (mbox_button_active==i)?COLOR_WHITE:COLOR_LIGHT_GREY);}
     else
        {cl = MAKE_COLOR((mbox_button_active==i)?COLOR_RED_P:COLOR_BLACK, (mbox_button_active==i)?COLOR_WHITE:COLOR_LIGHT_GREY_P);}
        ...
        draw_string(x+(((BUTTON_SIZE-strlen(lang_str(buttons[mbox_buttons[i]].text)))*FONT_WIDTH)>>1), mbox_buttons_y, lang_str(buttons[mbox_buttons[i]].text), cl,1);
...
void gui_mbox_draw() {
    if (mbox_to_draw) {
        ...
        if(gPlayRecMode>0)
         ...
         {draw_filled_rect(x-4, y-4, x+w*FONT_WIDTH+4, y+(h+2)*FONT_HEIGHT+SPACING_BTN+2+SPACING_TITLE+7, MAKE_COLOR(COLOR_GREY_P, COLOR_WHITE)); }
        ...
        l = strlen(mbox_title);
        draw_string(x+((w-l)>>1)*FONT_WIDTH, y, mbox_title, MAKE_COLOR(COLOR_BLACK, COLOR_WHITE),1); //title text
        ...
        while (h) {
          ...
          if(gPlayRecMode>0)
           {draw_string(x+d, y+h*FONT_HEIGHT, c[h], MAKE_COLOR(COLOR_GREY, COLOR_WHITE),1);} //text
          else
           {draw_string(x+d, y+h*FONT_HEIGHT, c[h], MAKE_COLOR(COLOR_GREY_P, COLOR_WHITE),1); }
        }
