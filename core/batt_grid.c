...
void gui_grid_draw_osd(int force) 
{
    struct gline  *ptr;
    char osd_buf[32];
    int delta;               // guideline offset from centre
    static int pz=0;         // previous zoom position
    static int cz=0;         // current zoom position
    coord xpos0;             // for vertical camera position moved 90 pixels left or right
    coord xpos1;             // for vertical camera position moved 90 pixels left or right
    int offset;       	     // next position for multiple vertical lines
...
// --------------------------------------------------------------------------------------------------------
             if((conf.camera_orientation!=1)&&(conf.camera_orientation!=3)) 
             {
              ...
              } // _horizontally mounted or inverted
// ---------------------------------------------------------------------------------------------------------
