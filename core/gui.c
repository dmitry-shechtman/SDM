...
extern int gZoomPoint,gFocalPoint,nTxzoom,nSW;
...
static CMenuItem sc_submenu_items[] = 
{
    ...
    {0,LANG_MENU_PERIOD_TEN_MSEC_COUNT,   MENUITEM_INT,&conf.SsTenMsecCount},
...
{
  {1,LANG_MENU_MAIN_SCRIPT_PARAM,         MENUITEM_PROC,   (int*)&load_easy_mode_script}, 
  {1,LANG_MENU_MAIN_TITLE,                MENUITEM_SUBMENU,      (int*)&root_menu},
  {0}
};

static CMenu user_submenu = {20,1, LANG_MENU_BASIC_TITLE, NULL, user_submenu_items };
...
// ***********************************************************
//                 Range limit the user-settable values
// ***********************************************************
...
// -------------------------------------------------------------------
//     Reyalp's menu enumeration CS988
// -------------------------------------------------------------------
...
void draw_mini_scripts()
{
 ...
 draw_string(xpos,ypos,filename,(!gPlayRecMode)?MAKE_COLOR(COLOR_BG_P,COLOR_RED_P):MAKE_COLOR(COLOR_BG,COLOR_RED),size);   
 }
 }
  sprintf(filename,"%04d",LowestCurCnt); 
...
// -----------------------------------------------------------------------------------------------------
	...
	if(!gSetPressed && get_usb_bit()&& gPlayRecMode && !state_kbd_script_run)
	  ...
	  else if((gSetPressed && (movie_status==4)&& !MODE_IS_SYNCH_VIDEO(vmode))\
...
synch_fps=SYNCH_FPS;	
#endif
        startTime=shooting_get_tick_count();
        long deltaPeriod;
  ...
  while(lvShow)
   {
    ...
    if(driftTime<2000)
     ...
     else if((driftTime>12000)&&(sync_speed>1))                               
     {
      sync_speed-=1;
      period2 = ((std_period+1+sync_speed)*1000000)/konst;						 
      deltaPeriod = period2-period1;
    ...
    if(driftTime && (driftTime<60000))
     ...
     draw_filled_rect(((screen_width>>1)-18), (screen_height>>1)-18, ((screen_width>>1)+18),(screen_height>>1)+18, MAKE_COLOR(COLOR_RED,COLOR_RED));
        ...
        lvShow=0;
        gSetPressed=0;
	if(gPlayRecMode==1)msleep(1500); // display header second line in Photo mode for 1.5 seconds
       gMenuPressed=keyMenuDuration=1;  // force exit from ALT mode
	   vid_bitmap_refresh();
       return;   
      }
  else   
   ...
   if(!driftDone)
   {
    ...
    if(driftTime && (driftTime<60000))
     {						
      ...
      while(shooting_get_tick_count()==dStart){};							 
      dStart=shooting_get_tick_count();
      *(volatile int*)(0xC0F06014) = (std_period)+SYNCH_SPEED; 					 
      *(volatile int*)(0xC0F06000) = 1;
      while (*(volatile int*)(0xC0F06000)) {};								 
      while((shooting_get_tick_count()-dStart)<driftTime){msleep(10);}				 
      *(volatile int*)(0xC0F06014) = std_period; 							 
...
if((conf.compact!=2)&&(recreview_hold==0)&& !inGridSelection())  
 {
  header_on = 1;
       /** -------------------------------------------------------------------
            header visible, not compact mode      
           -------------------------------------------------------------------*/
  if(!conf.compact)  
  {
...
if(!((conf.flag_4==1234)&& shooting_can_focus() && (displayedMode==23)))   
{
    ...
    // -------------------------------------------------------------------
    //   secondary header for SDM modes additional information    
    //   --------------------------------------------------------------------    
   ...
   if((conf.camera_orientation!=1)&&(conf.camera_orientation!=3))            
   {
    if((conf.user_1!=80973)&&(mode_video|| is_video_recording()|| (gui_mode==GUI_MODE_ALT)/*((gui_mode==GUI_MODE_ALT)&& mode_video &&(conf.video_synch_device==2))*/))  
    {
#if !defined(VIDEO_STD)
     if(conf.camera_position && ((!is_video_recording()&&MODE_IS_SYNCH_VIDEO(vmode))||(is_video_recording()&&!MODE_IS_SYNCH_VIDEO(vmode)))&& !kbd_blocked &&(LowestCurCnt!=9999)) 
#else		
     if(conf.camera_position && !is_video_recording()&& ((!kbd_blocked&&(gPlayRecMode==2))||(kbd_blocked&&(gPlayRecMode==1))) &&(LowestCurCnt!=9999)) 
#endif 
      {
       int tmp1,tmp2;
       ...
       if(tmp1<0)tmp1=STD_PERIOD+tmp1;        
       sprintf(osd_buf,"ISO %4d %4d/%4d->%4d",shooting_get_iso_market(),LowestCurCnt,tmp1,tmp2);
      }       
#if !defined(VIDEO_STD)
    else if(!conf.camera_position && ((!is_video_recording()&&MODE_IS_SYNCH_VIDEO(vmode))||(is_video_recording()&&!MODE_IS_SYNCH_VIDEO(vmode)))&& !kbd_blocked &&(LowestCurCnt!=9999)) 
#else		
    else if(!conf.camera_position && !is_video_recording()&& !kbd_blocked &&(LowestCurCnt!=9999)) 
#endif      
      {
 ...
 // ------------------------------------------------------   
  ...
  else 
  {
      ...
      else if((displayedMode>30)&&(displayedMode<34))    
     {
      ...
      {
       sprintf(osd_buf,"Zoom:%3s Focus:%3s %3s %3s %3s",(conf.remote_zoom_enable)?" ON":"OFF",(conf.dc)?" ON":"OFF",(conf.on_release)?" \x0A\x0B":" \x09\x09",(conf.save_raw)?"DNG":"",(conf.save_xml_file)?"XML":"");
      }
 
     else if(displayedMode==7) 
      {
       if(conf.user_1 !=80972)
        sprintf(osd_buf,"%19s%3s %3s %3s","",(conf.save_raw)?"DNG":"",(conf.save_auto)?"LOG":"",(conf.save_xml_file)?"XML":"");
       else
        sprintf(osd_buf,"%19s%3s %3s %3s","",(conf.save_raw)?"DNG":"",(conf.save_auto)?"LOG":"",(conf.save_xml_file)?"XML":"");       
     }
      ...
      {
       sprintf(osd_buf,"%2d.%01d sec interval  %3s %3s %3s",conf.FastLapseDelay/10,conf.FastLapseDelay%10,(conf.save_raw)?"DNG":"",(conf.save_auto)?"LOG":"",(conf.save_xml_file)?"XML":"");
      }
  ...
  else if((displayedMode==29)||(displayedMode==30))   
     {
      ...
      sprintf(osd_buf,"%18s %3s %3s %3s","",(conf.save_raw)?"DNG":"",(conf.save_auto)?"LOG":"",(conf.save_xml_file)?"XML":"");
     }   
    }  
   }  
   else if(!state_kbd_script_run)  
   {
   ...
   else  
    {
       ...
       if((gui_mode!=GUI_MODE_ALT)&&(((gHdrPhase==3)&&(displayedMode<=4)&&(conf.user_1!=80973))||(conf.user_1==80976))) 
        {
         if(gTooBright && (conf.user_1!=80976))                 
         {
          ...
          draw_string(xpos,conf.synch_pos.y+FONT_HEIGHT,osd_buf,MAKE_COLOR(COLOR_BG,COLOR_RED),1);
    ...
    else if((gui_mode!=GUI_MODE_ALT)&&(displayedMode>18)&&(displayedMode<23)&&(conf.user_1!=80973))  
     {
      ...
      //-------------------------------------------------------------------
     //  header visible,compact mode and time-lapse not running      
     //  --------------------------------------------------------------------     
...

  {
   if(header_on ) 
    ...
    /** -------------------------------------------------------------------
                    only display when shooting not in progress     
    --------------------------------------------------------------------*/
...
// ..........................................................................
//                      display override/bracketing legends
// ..........................................................................
 ...
 if(!inGridSelection())
 {
	...
	{
	 unsigned char xpos,ypos,size = 1;
	 int r;
...
static void gui_menuproc_reset_selected(unsigned int btn) 
{
    if (btn==MBOX_BTN_YES)
	{
	 ...
	 configVal.numb = 0;
     configVal.isNumb = 1;
     conf_setValue(47, configVal); // don't show splash
	}
}
