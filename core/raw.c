...
static char SerialNumber[30];
extern char *imgbuf;
extern unsigned char displayedMode,gSDM_will_use_flash,gImageCount,gEVfromScript;
...
int raw_savefile() 
{
  if(!shooting_get_drive_mode())
 {
  if(gImageCount==1)WriteFirmInfoToFile();
  else if(gImageCount==2)getSerialNumber();
 }
...
else if (conf.save_xml_file) 
 {
  ...
  if (fd>=0) 
   ...
   {   
    if(intValue<=50000) 
     {   
      xmlPrintf(fd,"Real shutter speed","1/%d",100000/intValue); // 1/2 or faster
     }
    ...
    else xmlPrintf(fd,"Real shutter speed","%2d.%1d",intValue/100000,((intValue+5000)%100000)/10000); // longer than one second
   }
   ...
   {       
     if(intValue<=50000) 
     {   
      xmlPrintf(fd,"Real override shutter speed","1/%d",100000/intValue); // 1/2 or faster
     }
    else if((intValue>50000) && (intValue<95000))        // 0.6 to 0.9 second
     {
    ...
    else xmlPrintf(fd,"Real override shutter speed","%2d.%1d",intValue/100000,((intValue+5000)%100000)/10000); // longer than one second
   }
