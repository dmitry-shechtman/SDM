...
// -------------------------------------------------------------------
/**
Sets fselect_title to file menu title and fselect_on_select to the callback
function for when file selected with SET.
current_dir set to current file directory.
Mode on entry saved in gui_fselect_mode_old and will be restored (normally) on exit.
Mode set to GUI_MODE_FSELECT so that file menu keyboard processing can start.
variables for file menu size (h,w) and position (x,y) initialised.
Create sorted linked-list of file structures
*/
...
void gui_fselect_init(int title, const char* dir, void (*on_select)(const char *fn)) {
   ...
   if((strcmp(dir,"A/SDM/SCRIPTS")==0)&& (strlen(dir)==13))
    ...
    if((conf.camera_orientation!=1)&&(conf.camera_orientation!=3))
     w = (1+nameSize+SPACING+SIZE_SIZE+SPACING+TIME_SIZE+1)*FONT_WIDTH;//filename,size,mod date and time (14 characters)
    else
     w = (1+nameSize+SPACING+SIZE_SIZE+SPACING+7)*FONT_WIDTH; // filename,size,mod date (eight characters)
...
for (i=0, ptr=top; i< num_lines && ptr; ++i, ptr=ptr->next) 
          {
            for (j=0; j<nameSize && ptr->name[j]; ++j) 
                buf[j]=ptr->name[j];
            if (j==nameSize && ptr->name[j]) buf[nameSize-1]='>'; 
...
// ----------------------------------------------- print directory path in footer ------
