        ...
        case PALETTE_MODE_SELECT:
            if (gui_palette_redraw) 
              ...
              for (y=0; y<16; ++y) 
                ...
                draw_filled_rect(0, FONT_HEIGHT, sw-1, FONT_HEIGHT+BORDER_SIZE-1, MAKE_COLOR(COLOR_GREY, COLOR_GREY)); //top
                draw_filled_rect(0, FONT_HEIGHT+BORDER_SIZE+16*CELL_SIZE+1, sw-1, FONT_HEIGHT+BORDER_SIZE+16*CELL_SIZE+BORDER_SIZE, MAKE_COLOR(COLOR_GREY, COLOR_GREY)); //bottom
                ...
                draw_filled_rect(sw-BORDER_SIZE, FONT_HEIGHT+BORDER_SIZE, sw-1, FONT_HEIGHT+BORDER_SIZE+16*CELL_SIZE, MAKE_COLOR(COLOR_GREY, COLOR_GREY)); //right
...
#if defined(STD_360_BUF)                   
    frame_buffer[0][yy * screen_buffer_width + (xx<<1)] = frame_buffer[1][yy * screen_buffer_width + (xx<<1)] = cl & 0xff;     
    frame_buffer[0][yy * screen_buffer_width + (xx<<1)+1] = frame_buffer[1][yy * screen_buffer_width + (xx<<1)+1] = cl & 0xff;

#elif  defined(CAMERA_sx1)
     for(offsetx=(((xx<<3)+xx)>>2);offsetx<=((((xx+1)<<3)+xx)>>2);offsetx++)                                                         
     {
      frame_buffer[0][yy*screen_buffer_width+offsetx]=frame_buffer[1][yy*screen_buffer_width+offsetx]=cl & 0xff;
     }
#else  
       frame_buffer[0][yy*screen_buffer_width+xx] = frame_buffer[1][yy*screen_buffer_width+xx] = cl & 0xff;                                                                                                                   
#endif
}
 ...
 void draw_pixel_grid(coord x,coord y,color cl)  
{
  unsigned int offsetx;
 #if defined(STD_360_BUF)                   
    frame_buffer[0][y * screen_buffer_width + (x<<1)] = frame_buffer[1][y * screen_buffer_width + (x<<1)] = cl & 0xff;     
    frame_buffer[0][y * screen_buffer_width + (x<<1)+1] = frame_buffer[1][y * screen_buffer_width + (x<<1)+1] = cl & 0xff;

#elif  defined(CAMERA_sx1)
     for(offsetx=(((x<<3)+x)>>2);offsetx<=((((x+1)<<3)+x)>>2);offsetx++)                                                         
     {
      frame_buffer[0][y*screen_buffer_width+offsetx]=frame_buffer[1][y*screen_buffer_width+offsetx]=cl & 0xff;
     }
#else  
       frame_buffer[0][y*screen_buffer_width+x] = frame_buffer[1][y*screen_buffer_width+x] = cl & 0xff;                                                                                                                   
#endif
}
...
#if  defined(CAMERA_sx1)
 return frame_buffer[0][y * screen_buffer_width + (((x<<3)+x) >>2)];
#else
