...
extern unsigned char displayedMode,gDidShowFocus,gNearFar,gSyncZoomStep,scriptZoomSize;
int PrevZoom;
// ******************************************************************************************************************************************
...
unsigned char clip8(signed short x){ if (x<0) x=0; else if (x>255) x=255; return x; } // CS841
 ...
 else
{
 ...
 if(zoom_busy)zcnt=30;
   ...
   if(((zoom != PrevZoom)||conf.zf_big_permanent)&&!gDidShowFocus) //zoom has changed or permanent but not rangefinder active
    {
	 if(bigZoomXPos && bigZoomYPos && state_kbd_script_run )
	  {
