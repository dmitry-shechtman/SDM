...
unsigned char gPrevStateVideoSynch,ConfRestoreFinished;
...
extern int LowestSynchCnt,pulseCount,movie_status,hdrLight96,hdrDark96;
...
{
 if((shooting_get_drive_mode()==1)&& shooting_in_progress()&& ((conf.fastlapse)||(gSDMShootMode==BURST)))
  {
...
#if defined(OPT_LOG)
 int logfd = open("A/SDMLOG.txt", O_WRONLY|O_CREAT, 0777);  	  
#endif
 SDMLOG(Spytask start\n);                               	        
    mkdir("A/SDM");

#ifdef OPT_EXMEM_MALLOC                                             
   SDMLOG(CAM_USES_EXMEM is TRUE\n);
#else
   SDMLOG(CAM_USES_EXMEM is FALSE\n);
#endif
...
SDMLOG(gui_init\n);
    gui_init();  
SDMLOG(gui_init finished\n);

SDMLOG(md_init\n); 
                                                           	
    md_init();   
SDMLOG(md_init finished\n);
    ...
    if(!beenInRec && ((mode_get()&MODE_MASK)==MODE_REC))
     {
      ...
      gPrevStateVideoSynch = MODE_IS_SYNCH_VIDEO(vmode);
     }
...
int m;
get_property_case(PROPCASE_SHOOTING_MODE, &m, 4);
m=shooting_mode_canon2chdk(m);
if(!conf.camera_position && movSynchMode && (conf.video_synch_device==1) && ((m==MODE_VIDEO_STD)||(m==MODE_VIDEO_SPEED)||(m==MODE_VIDEO_SUPER_SLOW)||(m==MODE_VIDEO_IFRAME_MOVIE)))
{
 volatile int* counter= PERIOD_COUNT;
 int std_period = STD_PERIOD;					
 int range;
int i;
 if(PERIOD_REF>=200)range = PERIOD_REF/100;
 else range = 2;

 static  int usbHi;
 static  int curCnt,cnt;
 while((gui_get_mode()==GUI_MODE_ALT))
{ 
 if(!flashing){flashing=1;gDisablePowerDown=1;}
 curCnt = *(counter) & 0xffff;     
 if((curCnt>=PERIOD_REF) && (curCnt<=(PERIOD_REF+range)))
  {
 ...
 else if(curCnt>std_period/2)
  {
   ubasic_set_led(9,0,0);
   if(!usbHi)usbHi=1;
  }
 }
 if(flashing){ubasic_set_led(9,0,0);flashing=0;gDisablePowerDown=0;}	
}
if(syncDone == 1)
 {
  char charbuf[40];
  int cnt,fd = open("A/PostSynchInfo.txt",O_WRONLY|O_CREAT, 0777);
       msleep(2000);
       if(fd>=0)
      {
        sprintf(charbuf,"Camera : %s\n\n",PLATFORM);
        write(fd,charbuf,strlen(charbuf));
        sprintf(charbuf,"Using %s for synch pulses\n\n",(conf.video_synch_device==1)?"Photo-sensor":(conf.video_synch_device==2)?"USB Switch":"None");
        write(fd,charbuf,strlen(charbuf));
        sprintf(charbuf,"Pulse count %d\n",pulseCount);
        write(fd,charbuf,strlen(charbuf));
        sprintf(charbuf,"Target phase %4d\n",PERIOD_REF);
        write(fd,charbuf,strlen(charbuf));
        sprintf(charbuf,"Phase before synch %d\n",temp1);
        write(fd,charbuf,strlen(charbuf));
        sprintf(charbuf,"Phase after synch %d\n",temp2);
        write(fd,charbuf,strlen(charbuf));
...
{
 short int i;
 short int* pBuf=pAFbuf;
       for(cnt=1;cnt<pulseCount;cnt++)
       {
        i=*pAFbuf;
        pAFbuf++;
        sprintf(charbuf,"%4d\n",i);
...
if(calibDone && ((movie_status==VIDEO_RECORD_STOPPED)||(movie_status==VIDEO_RECORD_NEVER_STARTED)))
 {
  char charbuf[40];
  int cnt, fd = open("A/calibration.txt",O_WRONLY|O_CREAT, 0777);
  short int i;
  short int* pBuf=pAFbuf;
