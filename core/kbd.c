...
unsigned char gRawMode,gVideoReady,ZoomIn=1,welcomeDone=0,forceNDout,gSyncZoomStep;
short kapbv96;
...
#elif defined(CAMERA_sx200is)||defined(CAMERA_sx240hs)||defined(CAMERA_sx260hs)
...
#if !defined(CAMERA_sx200is)&& !defined(CAMERA_sx240hs)&& !defined(CAMERA_sx260hs)
...
#if defined(CAMERA_sx240hs)||defined(CAMERA_sx260hs)||defined(CAMERA_g1x)||defined(CAMERA_ixus132)||defined(CAMERA_ixus140)
#define ZSTEP_TABLE_SIZE 9
...
void process_script()
{
          ...
          if(unlock || shooting_get_prop_int(PROPCASE_SHOOTING))
           {
            ...
            kbd_int_stack_ptr-=1; //pop wait op
           }
	    return;
 ...
 // Uses conf.user_2 (ecComp),conf.user_3 (preferred Av),conf.user_4 (min.Av)
 // conf.user_5 (bracketing offset)
 // ------------------------------------------------------------------------   
       case FAST_KAP: 
          {      
           kbd_int_stack_ptr-=1;
 ...
 if(get_usb_power(1))
  {
   ...
   if(pressedCount>40)
    {
 #if defined(CAMERA_sx200is)||defined(CAMERA_sx240hs)||defined(CAMERA_sx260hs)||defined(CAMERA_sx50hs)
...
// default f 0
// param g single/cont/time/burst/video
// default g 0
...
struct                
{
 ...
 int num_exposures;       
 int endless_mode;
 int shoot_mode;          // single,continuous,timer,burst
 ...
 int bracket_type;        // Tv,focus,both
 ...
 int focus_mode;          // equal-step,auto,digiscope
 int focus_step;
 int focus_reference; 
...
static unsigned char usbCount=0;
 ...
 if((tl.shutdown==2) && get_usb_power(1) && !usb_detect)                  
  ...
  if(tl.endless_mode && usbCount && !conf.flag_1 && (tl.shoot_mode!=4) && (tl.shutdown==2)&& !sd)                         
   ...
   else if((tl.shoot_mode==4) && usbCount  && (tl.shutdown==2) &&(movie_status==VIDEO_RECORD_STOPPED)&& !sd)                           
   { 
    sd=1;
    kbd_sched_delay(2000);  
    power_down();
 ...
 if(((gScriptFolder==1) && (strncmp(conf.script_file,"A/SDM/SCRIPTS2/K_.txt",18)==0))||(!gScriptFolder && (strncmp(conf.easy_script_file,"A/SDM/SCRIPTS/KAP.txt",18)==0))) 
 {
  ...
  if((tl.shoot_mode==4)&&(movie_status ==VIDEO_RECORD_STOPPED) && tl.blank_screen)
   {
    TurnOffBackLight();
    bls=0;
   }
...
void usb_high_count()            // called by precision timer or high priority bgndBeeper() task            
{
  static int USBcnt;
...
//Called from my_kbd_read_keys(). Returns true to override keys (physw_status), false to leave them alone.
...
unsigned int drmode = 0;
 ...
 if(skey_pressed)
     ...
     if(!mplay &&!canon_shoot_menu_active && (canon_menu_active==(int)&canon_menu_active-4))          
      {
       ...
       if(kbd_is_key_pressed(KEY_MENU)&&!kbd_is_key_pressed(KEY_SHOOT_HALF)&&(gui_mode==GUI_MODE_NONE) &&!canon_shoot_menu_active && (canon_menu_active==(int)&canon_menu_active-4))
        {
                key_pressed = 1;
                kbd_key_release_all();          
                return 1;
...
// ---------------------------------------------------------------------------------------------------------------------
//                         Half-press button on startup on some cameras to switch to Record mode
// ---------------------------------------------------------------------------------------------------------------------
...
if(mplay && kbd_is_key_pressed(conf.anaglyph_button)&& conf.stereo_mode && (displayedMode==6)) 
{
 nWt=100;
 kbd_key_release_all();      
 stereo_pair();
...
if(nFirst==1)
{
    ...
    if((nSW==11)&& conf.remote_zoom_enable && !nCount2)     
    {
     nSW=0;
     gSZMode=0;
    ...
    if((nSW==12)&& conf.remote_zoom_enable && !nCount2 && !is_video_recording())     
    {
      ...
      nWt=50;
      shutter_int=0;  
      if(((movie_status==VIDEO_RECORD_STOPPED)||(movie_status==6))&& conf.video_frame_count && (gFrameCount<=2) && mvideo){gFrameCount=conf.video_frame_count;}
      else if(((movie_status==VIDEO_RECORD_STOPPED)||(movie_status==6))&& !conf.video_frame_count && mvideo){gFramesRecorded=0;} 
      if(!(srf && !conf.srb))
#if !defined(CAM_HAS_VIDEO_CONTROL_BUTTON)
       kbd_key_press(KEY_SHOOT_FULL);  
#else
 ...
 unsigned char cond1,cond2;
  ...
  cond2 = (gSDMShootMode==7)&&(mvideo||is_video_recording())&& !nTxzoom;     
   ...
   kbd_key_press(KEY_SHOOT_FULL);   
	...
	if(nCa==2)
		...
		if(nSW==114)
			...
			if(!(srf && !conf.srb))
                   {
                    if((movie_status==VIDEO_RECORD_STOPPED)&& conf.video_frame_count && (gFrameCount<=2) && mvideo){gFrameCount=conf.video_frame_count;}
#if !defined(CAM_HAS_VIDEO_CONTROL_BUTTON)
           kbd_key_press(KEY_SHOOT_FULL);                                           
#else         
            if(mvideo)
