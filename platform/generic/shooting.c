...
int shooting_get_farpoint(int distance)                    // returns -1 if at or greater than hyperfocal
{
 int hyp = onHalfPress.Hyperfocal;
 if(!fpd || (distance>=MAX_DIST))return -1;  
 unsigned int num = (hyp*1000)/(hyp-distance);  // num x 1000
...
{
 unsigned int flx1000 = get_focal_length(lens_get_zoom_point()); // fl x 1000
