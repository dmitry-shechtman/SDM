...
#define RECORD 0x1001        // "PressRecButton"
#define PLAYBACK 0x1003      // "PressPBButton"
#define MOVIE 0x105B         // "ModeDialToMovie"

#elif !CAM_HAS_PLAYBACK_PB 
#define RECORD 0x1065       // search "ModeDialToRec"
#define PLAYBACK 0x1061     // search "ModeDialToPlay"
#define MOVIE 0x105B        // search "ModeDialToMovie"
// #endif
#elif CAM_HAS_PLAYBACK_PB 
#define RECORD 0x1001    // search "PressRecButton"
#define PLAYBACK 0x1003  // search "PressPBButton"
#define MOVIE 0x9A5      // search "PressMovieButton"
#endif

#define LEVENT_INVALID_INDEX 0xFFFFFFFF

#if CAM_MULTIPART
extern int cp;  // current partition '1' or '2'
#endif
...
void get_standard_period()
{
 ...
 std_period = *(volatile int*)(0x00606014); 		// shadow ram,value set by camera, 0x3C2 in normal lighting
#else
 std_period = *(volatile int*)(0x00406014); 		// shadow ram,value set by camera, 0x110 in normal lighting
#endif
...
void fps(int x)
{
 current_fps+=x;
#if defined (CAMERA_s100)
 *(volatile int*)(0x00606014)= current_fps;		// shadow ram
#else
 *(volatile int*)(0x00406014)= current_fps;		// shadow ram
#endif
...
void __attribute__((weak)) vid_bitmap_refresh()   // CS723, EWAVR
{
...
void lens_set_focus_pos(long newpos)    // from sensor
{
...
{
  _RefreshUSBMode();                                                     // and refresh mode
}
...
long __attribute__((weak)) get_jogdial_direction(void)
{
 return 0;
	...
	void play_sound(short sound) 
 		{ 
 		    short tmp = 0xC211; // shutup compiler
 		    switch (sound) 
 		{ 
 		  case 0: 
...
int GetMechaPos()
{
 ...
 if(!registered)
  {
   if(_ExecuteEventProcedure("Mecha.Create") == -1) 
    {if(_ExecuteEventProcedure("MechaRegisterEventProcedure") == -1)return 0;}
    registered=1;
  }
  return _ExecuteEventProcedure("GetFocusLensCurrentPosition");
}
 ...
 if (fd>=0)                              // if created and opened OK
  {  
   if(_ExecuteEventProcedure("UI.CreatePublic") == -1) 
    ...
    play_sound(4);
    for(i=0x8000;i<0x8020;i++)                   // for prop cases 0 to 300
       {
        id = _ExecuteEventProcedure("PTM_GetCurrentItem",i);
        sprintf(mybuf,"0x%04x %d\n",i,id);     // get property case
        write(fd,mybuf,strlen(mybuf));                               // write to file
       } 
       close(fd);                        // close file
       filenum++;                    
  }
...
/*
// This version does not register but address of _PTM_GetCurrentItem() must be in stubs.
// Used with G7x as above version crashed.
...
int start_self_timer()                            // return 0 if timer already running or error,  1 if successful.Units of HPTimer.units usec
{
 int duration = HPTimer.units;
    ...
    return 0;   // already running or error
}
