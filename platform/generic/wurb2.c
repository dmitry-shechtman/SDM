  ...
  if (conf.synch_delay_enable)				
     {
      ...
      if (target_count) 					
       ...
       {
        HPTimer.units=100000;                      // units of 0.1 seconds
...
if((get_tick_count()>5000)&&(!gSDM_will_use_flash|| (gSDM_will_use_flash&&conf.precision_synch_flash))&&! shooting_get_drive_mode())
{
	int std_period = STD_PERIOD; 					 
	int cur_cnt = *(counter) & 0xffff; 				// get the counter state at the time of sync
