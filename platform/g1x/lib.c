...
long hook_raw_size()             { return 0x01941be8; }
...
{
 static char led_table[7]={0,0,1,1,3,9,3};
...
void JogDial_CW(void){
 _PostLogicalEventToUI(0x872, 1);  // RotateJogDialRight (in levent_table)
}

void JogDial_CCW(void){
 _PostLogicalEventToUI(0x873, 1);  // RotateJogDialLeft (in levent_table)
}
...
{
    if (MODE_IS_VIDEO(mode_get()) || (movie_status==VIDEO_RECORD_IN_PROGRESS) || (movie_status==VIDEO_RECORD_IN_PROGRESS))
...
static int vp_yoffset(int stitch)
{
    ...
    else if (MODE_IS_VIDEO(mode_get()) || (movie_status==VIDEO_RECORD_IN_PROGRESS))
    {
        return 30;
    }
    else
