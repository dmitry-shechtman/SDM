...
typedef struct {
	...
	// test for dialmode-switch and playback/rec-switch
	typedef struct {
		short fun;      
		long hackkey;
...
#define SWITCH_MASK0 (0x00000005)
#define SWITCH_MASK1 (0x00000000)
#define SWITCH_MASK2 (0x00005000)
...
else if(ub || sertx)                                      // skip script processing in \core\kbd.c but allow key override
 {
   physw_status[0] = (kbd_new_state[0] & (~KEYS_MASK0)) |
     (kbd_mod_state[0] & KEYS_MASK0);
...
static SwitchMap switchmap[] = {
        { 1, MOVIE_MODE         , 0x00000005, 0x00000000, 0x00005000 },
        { 1, PLAYBACK_MODE      , 0x00000000, 0x00000000, 0x00001000 },
	  { 0, 0, 0, 0, 0 }
};
