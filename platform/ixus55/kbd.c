...
platform_kbd_hook();
	...
	physw_status[2] = (kbd_new_state[2] & (~0xaffe)) |
      ...
      if (switch_override == 1) 
        ...
        {
          physw_status[1] = ((physw_status[1] & (~SWITCH_MASK1)) | (switch_mod_state[1] & SWITCH_MASK1))& 0xfffffffd;
        }
         ...
         if (switchmap[i].hackkey == key)
           {
             ...
             switch_mod_state[1] &= ~switchmap[i].canonkey1;  // playback
             switch_mod_state[2] &= ~switchmap[i].canonkey2;
             return;
           }
         }
...
static SwitchMap switchmap[] = 
 {
        ...
        { 1, PLAYBACK_MODE, 0x00000000, 0x00000002, 0x00000000 }, 
	  { 0, 0, 0, 0, 0 }
 };
