...
----------------------------------------------------------------------*/
 ...
 #define TASK_LOG_MAX 80000
 
 volatile int task_log = 0;
 volatile int task_log_start = 0;
 volatile unsigned int task_log_idx = 0;
 volatile char *task_log_buf = NULL;
 
 int task_log_file_num = 0;
 
 void task_log_init()
 {
 	if (!task_log_buf) task_log_buf = umalloc(TASK_LOG_MAX + 1000);
 }
 
 void task_log_flush()
 {
 	if (task_log_idx && !task_log)
 		{
 		char fn[12];
 		int fd;
 		...
 		sprintf(fn, "A/sw%05d.txt", task_log_file_num);
 		task_log_file_num++;
 
 		fd = open(fn, O_WRONLY|O_CREAT, 0777);
 		if (fd>=0)
 			{
 			write(fd, task_log_buf, task_log_idx);
 			close(fd);
 			}
 		task_log_idx = 0;
 		}
 ...
 volatile int preparing_for_capture = 0;
 
 void _prepare_wait_until_remote_button_is_released(void)
 {
 	preparing_for_capture = 1;
 	
 	task_log_start = (int)get_tick_count();
 	task_log_idx = 0;
 	task_log = 1;
 }
 
 void __attribute__((naked,noinline)) prepare_wait_until_remote_button_is_released(  ) 
 { 
 asm volatile (
 ASM_SAFE("BL _prepare_wait_until_remote_button_is_released\n")
  "	BX      LR \n"
 	) ;
 }
...
void taskCreateHook(int *p)
{
	...
	if (p[0] == 0xFF8CF1A8)
 	...
 	if (preparing_for_capture && strcmp((char *) p[6], "TgTask") == 0)
 		{
 		if (task_log && task_log_buf && task_log_idx < TASK_LOG_MAX)
 			{
 			task_log_buf[task_log_idx++] = 'U';
 			task_log_buf[task_log_idx++] = '0' + get_usb_bit();
 			task_log_buf[task_log_idx++] = 0x0a;
 			}
 			
 		wait_until_remote_button_is_released();
 		preparing_for_capture = 0;
 	...
 	if (task_log && task_log_buf && task_log_idx < TASK_LOG_MAX) 
   {
 		...
 		int dt = tick-task_log_start;
 		
 		if (dt > 2000 || task_log_idx > TASK_LOG_MAX) 
      {
 			task_log = 0;
 		}
 		else 
      {
 			task_log_buf[task_log_idx++] = '0' + (dt / 1000) % 10; // this is supposed to be faster than sprintf
 			task_log_buf[task_log_idx++] = '0' + (dt / 100) % 10;
 			task_log_buf[task_log_idx++] = '0' + (dt / 10) % 10;
 			task_log_buf[task_log_idx++] = '0' + (dt     ) % 10;
 			task_log_buf[task_log_idx++] = ' ';
 			int i;
 			char *ptr = (char *) p[6];
 			for (i = 0; i < 16; i++) {
 				if (!*ptr) break;
 				task_log_buf[task_log_idx++] = *ptr++;
 			}
 			task_log_buf[task_log_idx++] = 0x0a;
 		}
 	}
