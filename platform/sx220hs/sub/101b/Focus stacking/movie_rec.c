...
extern unsigned int focus_table[];
...
if(VideoMode==VIDEO_BURST){if((gFrameCount--)<2)gStopVideoCapture=1;}
   if(gFramesRecorded==1)index=0;
   if(!(gFramesRecorded%2)&&focus_table[0]&&conf.video_frame_count){_MoveFocusLensToDistance((short*)&focus_table[index]);index++;}
...
void __attribute__((naked,noinline)) sub_FF2FB378_my(  ) {
asm volatile (
                "STMFD  SP!, {R0-R12,LR}\n"
                "MOV    R9, R0\n"
                "LDR    R0, [R2, #0x10]\n"
