...
{
 int vmode;
 vmode = shooting_get_prop_int(PROPCASE_SHOOTING_MODE);
 if(vmode!=3650)inc=0;                       // only standard video supported                     
 else
 {
  vmode = shooting_get_prop(PROPCASE_VIDEO_RESOLUTION);
  if(vmode==2)inc=0;                         // 640x480 already counted in set_quality()
  else
  {
   if(vmode==5)inc = 24;							// 1920x1080
   else inc = 30;                            // 1280x720
  }
 }
}
void frameCounter()
{
 if(inc)
  {
   gFramesRecorded+=inc;
   if(VideoMode==VIDEO_BURST)
   {
    if((gFrameCount-=inc)<2)gStopVideoCapture=1;
   }
  }
}
*/
...
"loc_FF081480:\n"				         // on power-up, changing dial to Movie, starting movie with dial in non-movie mode.
"    STR     R6, [R4, #0x40] \n"
"    STR     R6, [R4, #0x34] \n"
"    STR     R7, [R4, #0x148] \n"
"    STRH    R6, [R4, #6] \n"
...
"loc_FF0814D0:\n"                // on power-up, changing dial to Movie, starting movie with dial in non-movie mode (enter twice).
"    STR     R5, [R4, #8] \n"
"    B       loc_FF08159C \n"
...
"loc_FF081574:\n"                // on stopping, all modes
"    BL      sub_FF07ED64 \n"
"    B       loc_FF08159C \n"

"loc_FF08157C:\n"                // on stopping, HD modes
"    BL      sub_FF08202C \n"
"    B       loc_FF08159C \n"
