...
#define SWITCH_MASK1 (0xf8000000)
...
platform_kbd_hook();
      ...
      if (switch_override == 1)                                // only movie mode supported
        { 
          physw_status[0] = (physw_status[0] & (~SWITCH_MASK1)) | (switch_mod_state[1] & SWITCH_MASK1);  
...
void kbd_key_release(long key)
{
    ...
    { // dialmode-switch
       ...
       for (i=0; switchmap[i].hackkey; i++)
        {
          if (switchmap[i].hackkey == key)                    // **** only movie mode supported  *****
           {
...
static SwitchMap switchmap[] = {
        { 1, MOVIE_MODE, 0x00000000, 0x98000000, 0x00000000 },
	  { 0, 0, 0, 0, 0 }
};
